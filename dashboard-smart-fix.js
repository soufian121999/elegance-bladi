(()=>{
  ...
  const paymentText=x=>norm([parcel(x)?.payment_status,parcel(x)?.payment_status_label,parcel(x)?.ozon_payment_status,parcel(x)?.ozon_payment_status_label,parcel(x)?.status_payment,parcel(x)?.status_payment_label,x?.payment_status,x?.payment_status_label].filter(Boolean).join(' '));
  const isFactured=x=>paymentText(x).includes('factur');
  ...
  async function tariffs(){try{let j;try{j=await fetch(EB.API+'/cities',{cache:'no-store'}).then(r=>r.json())}catch{j=await fetch('https://api.ozonexpress.ma/cities',{cache:'no-store'}).then(r=>r.json())}const map=new Map();cityRecords(j?.data??j).forEach(o=>{const n=pick(o,['CITY_NAME','city_name','name','ville','city','label']),d=pick(o,['DELIVERED-PRICE','DELIVERED_PRICE','delivered_price','delivery_price','frais_livraison','livraison']),r=pick(o,['REFUSED-PRICE','REFUSED_PRICE','refused_price','RETURNED-PRICE','RETURNED_PRICE','returned_price','return_price','frais_refus','refus']);if(n)map.set(norm(n).trim(),{delivered:num(d),refused:num(r)})});return map}catch{return new Map()}}
  function feeFor(x,map,type){const names=[x?.city_name,x?.city,parcel(x)?.city_name,parcel(x)?.city].filter(Boolean);for(const n of names){const t=map.get(norm(n).trim());if(t)return num(type==='delivered'?t.delivered:t.refused)}return 0}
  ...
  async function refresh(){try{
    ...
    const feeMap=new Map();[...plainReturn,...refRet,...annRet,...currentRef,...currentAnn].forEach(x=>{const tr=String(parcel(x)?.tracking_number||x?.id||Math.random());if(!feeMap.has(tr))feeMap.set(tr,x)});const feeOrders=[...feeMap.values()];const returnFees=feeOrders.reduce((a,x)=>a+feeFor(x,tar,'refused'),0);const netDue=grossDue-deliveryFees-returnFees;
    ...
    setHero(0,'Net estimé à recevoir de Ozon',`${fmt(netDue)} DH`,`${nonFactured.length} Livré Non Facturé`,`<div class="moneyBreak"><div><span>COD brut en attente</span><strong>${fmt(grossDue)} DH</strong></div><div><span>− Frais livraison</span><strong>${fmt(deliveryFees)} DH</strong></div><div><span>− Frais refus/retour (${feeOrders.length})</span><strong>${fmt(returnFees)} DH</strong></div></div>`);
    ...
    setMini('Livré',delivered.length,`${delivered.filter(x=>today(parcel(x)?.updated_at||x.updated_at)).length} livrées aujourd'hui`);setMini('En cours',progress.length,'Chez Ozon');
